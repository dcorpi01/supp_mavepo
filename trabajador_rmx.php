...
session_start();
    ...
    if(!empty($_POST)){
        ...
        if(empty($_POST['nombre']) || empty($_POST['apep']) || empty($_POST['apem']) || empty($_POST['curp']) || empty($_POST['rfc']) || empty($_POST['muni'])){
            ...
            $save = "INSERT INTO trabajadormx(nombre, apep, apem, dptto, curp, puesto, ec, nss, rfc, calle, numint, numext, col, cp, muni, hr, tipoem, fechain, edad, fechanac) VALUES ('$nombre', '$apep', '$apem', '$dptto', '$curp', '$puesto', '$ec', '$nss', '$rfc', '$calle', '$numint', '$numext', '$col', '$cp', '$muni', '$hr', '$tipoem', '$fechain', '$edad', '$fechanac')";
